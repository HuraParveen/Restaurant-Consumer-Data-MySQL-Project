...
-- CREATING CONSUMER_PREFERENCES TABLE
...
DROP TABLE consumer_preferences;
...
-- CREATING RESTAURANT_CUSINES TABLE
...
WHERE c.Consumer_ID NOT IN (
    ...
    FROM ratings ra2
    JOIN restaurant_cuisines rc ON ra2.Restaurant_ID = rc.Restaurant_ID
...
SELECT DISTINCT c.Consumer_ID, c.Age FROM consumers c JOIN ratings ra ON c.Consumer_ID = ra.Consumer_ID JOIN restaurants r ON ra.Restaurant_ID = r.Restaurant_ID WHERE c.Drink_Level = 'Social Drinker' AND r.Parking = 'No';


-- Ephasizing WHERE Clause and Order of Execution
...
SELECT ra.Consumer_ID, COUNT(*) AS Rated_Count
FROM ratings ra
...
WHERE c.Occupation = 'Employed'
  ...
  -- Derived Tables, CTEs, Window Functions, Views, Stored Procedures
...
SELECT Consumer_ID, Restaurant_ID, Overall_Rating,
       AVG(Overall_Rating) OVER (PARTITION BY Consumer_ID) AS Consumer_Avg
FROM ratings;
...
WITH mexican_consumers AS (
    SELECT Consumer_ID
    FROM consumer_preferences
    WHERE Preferred_Cuisine = 'Mexican'
)
SELECT mc.Consumer_ID
FROM mexican_consumers mc
WHERE mc.Consumer_ID NOT IN (
    SELECT Consumer_ID
    FROM ratings
    WHERE Restaurant_ID IN (
        SELECT Restaurant_ID FROM HighlyRatedMexicanRestaurants
    )
);
...
CREATE PROCEDURE GetRestaurantRatingsAboveThreshold(
    ...
    IN min_Overall_Rating DECIMAL(3,1)
)
BEGIN
    SELECT 
        Consumer_ID,
...
CREATE VIEW ConsumerAverageRatings3 AS
SELECT
    Consumer_ID,
...
CREATE PROCEDURE GetConsumerSegmentAndRestaurantPerformance4 (
    IN cid VARCHAR(20)
)
BEGIN
    /* 1️⃣ Consumer Spending Segment */
    SELECT
        Consumer_ID,
        Budget,
        CASE
    ...
    SELECT
        ...
        avg_r.Restaurant_Avg_Rating,
        CASE
            WHEN ra.Overall_Rating > avg_r.Restaurant_Avg_Rating THEN 'Above Average'
            WHEN ra.Overall_Rating = avg_r.Restaurant_Avg_Rating THEN 'At Average'
            ELSE 'Below Average'
...
CALL GetConsumerSegmentAndRestaurantPerformance1('U1001');
